...
function resolvePiAiCliPath() {
  ...
  while (true) {
    const candidate = path.join(currentDir, "node_modules", "@mariozechner", "pi-ai", "dist", "cli.js");
    if (existsSync(candidate)) {
      return candidate;
    }
    const parentDir = path.dirname(currentDir);
    if (parentDir === currentDir) {
      throw new Error("Could not locate the installed @mariozechner/pi-ai CLI.");
    }
...
async function loadPiAiCredentials(tempDir: string): Promise<OAuthCredentials> {
  ...
  if (!credentials || typeof credentials.access !== "string" || typeof credentials.refresh !== "string" || typeof credentials.expires !== "number") {
    throw new Error("pi-ai login did not produce usable openai-codex credentials.");
  }
...
export async function loginCodex(authFilePath: string) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "immanence-pi-ai-auth-"));
  try {
